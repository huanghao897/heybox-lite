...
已实现 `feed.list`、`post.detail`、`comment.list` 和 `search.links` 四个只读操作。App 只发送逻辑操作名、经过白名单约束的业务参数和当前 Cookie；服务器根据最终官方路径生成 LEGACY 签名，并将官方 JSON 与 `Set-Cookie` 返回给 App。

远程开关 `gateway_read_test` 仅对固定测试管理员生效，`gateway_read` 对全部用户生效，默认均为关闭。二维码登录、用户动态、历史、收藏列表、官方表情和全部写操作仍保持原链路，图片及动图继续直连 CDN。
