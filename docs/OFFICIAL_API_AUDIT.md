...
Lite 的登录 Cookie 包含 `pkey`、`x_pkey`、`x_xhh_tokenid` 和账号标识。这些字段
一旦发送给自建服务器，服务器就具备脱离客户端复用用户身份的能力；即使只开放
固定只读操作、只在内存中转发，也不能消除服务器失陷或域名劫持后的账号风险。

因此小黑盒业务接口固定由客户端直连官方域名，远程配置不能切换到 Cookie 中转。
后台只接收独立的设备心跳令牌、版本信息和有界遥测，不接收小黑盒 Cookie、
`pkey`、`x_xhh_tokenid`、Authorization 或签名请求参数。

如果未来需要服务端热更新接口适配，只能使用官方授权的服务端凭据，或使用无法被
服务器复用为任意请求的一次性签名协议；在没有这种协议前，不恢复凭据中转。
