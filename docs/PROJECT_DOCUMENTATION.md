...
用户已经明确要求：每次更新项目或修改代码后，都要使用固定签名构建 release APK，确认构建和签名校验通过，再提交本次相关改动并推送到当前 GitHub 分支。输出到：
...
如果构建失败，不得推送未验证的代码。默认只推送代码提交，不创建 GitHub Release，也不上传 APK 附件；只有用户明确要求发布版本时才操作 Release。用户明确说“先别传”或“不用上传”时，暂停 GitHub 提交与推送。服务器发布仍需用户单独授权。
...
- bug 数量较多时更新版本号；
- 新功能较多时更新版本号；
- 每次更新项目或修改代码后都必须构建签名 release APK；
- 构建和签名校验通过后，必须提交本次改动并推送当前 GitHub 分支；
- 默认不创建 GitHub Release，也不上传 APK 附件；
- 只有用户明确说“先别传”或“不用上传”时才暂停 GitHub 推送；
- 服务器上传需要用户明确说“传服务器”；
- GitHub 推送与服务器发布是两件事，不能因为推送 GitHub 而自动发布到服务器。
