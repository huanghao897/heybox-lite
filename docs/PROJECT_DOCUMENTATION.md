...
面向后续维护、构建、发布与排查的工程说明。目标是让接手者能快速理解项目结构、核心流程与风险点，少踩坑。所有内容以仓库当前代码为准（版本 `2.0.4` / versionCode `207`）。
...
| `namespace` / 包名 | `com.openzen.heyboxcommunity` |
| `versionName` / `versionCode` | `2.0.4` / `207` |
...
│  └─ src/main/
│     ├─ AndroidManifest.xml
│     ├─ java/com/openzen/heyboxcommunity/   # 全部源码
│     └─ res/
...
全部源码约 40 个类，位于 `app/src/main/java/com/openzen/heyboxcommunity/`。下面按职责分组。
...
- **`PresenceReporter`** — 前台在线心跳。同进程最短 60 秒一次；只上报**公开身份**（登录时的用户 id/昵称/头像）与设备 id、版本、versionCode、机型、系统版本；**不携带** Cookie/pkey/token/签到凭据/评论或浏览内容。`MainActivity` 前台每 2.5 分钟触发一次以维持在线窗口，退后台即停。
...
> **在线统计为双轨**：`presence`（IP 级）用于兜底与访客视角；`presence_users`（用户/设备级，2.0.4 起 App 上报公开身份后启用）用于更精准的「当前在线 / 注册用户数 / 累计设备 / 今日活跃」。管理后台的用户视图主要基于 `presence_users`。在线窗口 300 秒，客户端心跳最短 60 秒。
